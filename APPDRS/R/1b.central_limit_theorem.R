Teorema_do_Limite_Central <- function(){
  ...
  ui <- fluidPage(
    ...
    titlePanel(h3("Teorema do Limite Central - Curso Desvendando o R na Saúde (Módulo 1)")),
    sidebarLayout(
      sidebarPanel(
  ...
  server <- function(input,output,session){
    ...
    output$jp_graph<-renderPlot({ hist( CLT( freq_yes(), freq_no(), n_each_sam(), amount_sam()), ylab="Frequência",xlab="Porcentagem da Amostra", main="Distribuição de Amostragem" )
                                abline(v= pop_porc()+(    jp_pop_sd(CLT( freq_yes(), freq_no(), n_each_sam(), amount_sam()))     * 2), col='red', lwd=3,lty='dashed')
                                abline(v= pop_porc()-(    jp_pop_sd(CLT( freq_yes(), freq_no(), n_each_sam(), amount_sam()))     * 2), col='red', lwd=3,lty='dashed')
