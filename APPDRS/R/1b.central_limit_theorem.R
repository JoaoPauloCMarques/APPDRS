Teorema_do_Limite_Central <- function(){
  ...
  server <- function(input,output,session){
    ...
    pop_prop <- eventReactive(input$btn,{   round( ( freq_yes()/sum(freq_yes(),freq_no()) ) , digits=2)})
    ...
    output$jp_graph<-renderPlot({ hist( CLT( freq_yes(), freq_no(), n_each_sam(), amount_sam()), ylab="Frequência",xlab="Porcentagem da Amostra", main="Distribuição de Amostragem" )
                                #abline(v= pop_porc()+(sd_of_sampling_prop_dis( freq_yes()/sum(freq_yes(),freq_no()) ,amount_sam()) * 2), col='red', lwd=3,lty='dashed')
                                #abline(v= pop_porc()-(sd_of_sampling_prop_dis(freq_yes()/sum(freq_yes(),freq_no()),amount_sam()) * 2), col='red', lwd=3,lty='dashed')
